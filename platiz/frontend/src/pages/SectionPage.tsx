      ...
      {textGuide && (
        <>
          <button
            ...
            className="fixed bottom-6 right-6 z-50 flex items-center gap-2 px-5 py-3 bg-[#FFD700] hover:bg-[#FFE44D] text-black font-bold rounded-full shadow-lg shadow-[#FFD700]/20 transition-all hover:scale-105"
          >
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M2 3h6a4 4 0 014 4v14a3 3 0 00-3-3H2z"/><path d="M22 3h-6a4 4 0 00-4 4v14a3 3 0 013-3h7z"/></svg>
            Guia PLR
          </button>
          ...
          {modalOpen && (
            <div className="fixed inset-0 z-[100] flex items-center justify-center p-4" onClick={() => setModalOpen(false)}>
              ...
              <div className="relative z-10 w-full max-w-3xl max-h-[85vh] overflow-y-auto glass rounded-2xl p-8 border border-[#FFD700]/20" onClick={(e) => e.stopPropagation()}>
                <div className="flex justify-between items-start mb-6">
                  <h2 className="text-2xl font-bold text-white">{textGuide.title}</h2>
                  <button onClick={() => setModalOpen(false)} className="text-gray-400 hover:text-white text-2xl leading-none">&times;</button>
                </div>
