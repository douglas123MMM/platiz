    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <HiUsers className="w-8 h-8 text-[#E5C158]" />
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={() => setShowForm(false)}>
          <div className="w-full max-w-lg glass rounded-3xl p-8 border border-[#E5C158]/10" onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true" aria-label={editId ? 'Editar Socio' : 'Nuevo Socio'}>
      ...
      <div className="glass rounded-2xl border border-[#E5C158]/10 overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead><tr className="border-b border-[#E5C158]/10">
              ...
              {loading ? (<tr><td colSpan={4} className="p-12 text-center text-gray-500">Cargando...</td></tr>) : partners.length === 0 ? (
                ...
                <tr key={p.id} className="border-b border-[#E5C158]/5 hover:bg-[#E5C158]/5 transition-colors">
                  <td className="p-4"><div className="flex items-center gap-3">
                    {p.photo_url ? <img src={p.photo_url} alt={p.name} className="w-10 h-10 rounded-full object-cover ring-2 ring-[#E5C158]/20" /> : <div className="w-10 h-10 bg-gradient-to-br from-[#C4A44A] to-[#A6842C] rounded-full flex items-center justify-center text-black font-bold text-sm">{p.name.charAt(0)}</div>}
                  ...
                  <td className="p-4 text-right">
                    <div className="flex items-center justify-end gap-2">
                      <button onClick={() => toggleActive(p.id)} className="p-2 rounded-lg bg-[#E5C158]/10 text-[#E5C158] hover:bg-[#E5C158]/20 transition-colors"><HiSwitchHorizontal className="w-4 h-4" /></button>
