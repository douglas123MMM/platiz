  ...
  return (
    <div className="space-y-8 animate-fade-in">
      <div className="flex items-center gap-3">
        <IconSearch className="w-8 h-8 text-[#E5C158]" />
        <div>
      ...
      <form onSubmit={handleSearch} className="flex items-center gap-3 px-4 py-3 glass rounded-2xl border border-[#E5C158]/10 max-w-2xl">
      ...
      {loading ? (
        <div className="text-center py-12">
          <div className="w-10 h-10 border-2 border-[#E5C158]/30 border-t-[#E5C158] rounded-full animate-spin mx-auto" />
        </div>
      ) : query && totalResults === 0 ? (
        <div className="text-center py-16">
          <IconSearch className="w-16 h-16 text-gray-600 mx-auto mb-4" />
          <p className="text-gray-500 text-lg">Sin resultados para "<strong className="text-[#E5C158]">{query}</strong>"</p>
        ...
        <div className="space-y-10">
          {results.users.length > 0 && (
            <div>
              <div className="flex items-center gap-2 mb-4">
                <IconUsers className="w-5 h-5 text-[#E5C158]" />
                  ...
                  <tbody>
                    {results.users.map((user) => (
                      <tr key={user.id} className="border-b border-[#E5C158]/5 hover:bg-[#E5C158]/5 transition-colors">
                        <td className="p-4">
                          <Link to="/admin/users" className="flex items-center gap-3 hover:text-[#E5C158] transition-colors">
                            <div className="w-8 h-8 bg-gradient-to-br from-[#C4A44A] to-[#A6842C] rounded-lg flex items-center justify-center text-black font-bold text-xs">{user.username.charAt(0).toUpperCase()}</div>
          ...
          {results.items.length > 0 && (
            <div>
              <div className="flex items-center gap-2 mb-4">
                <IconCourses className="w-5 h-5 text-[#E5C158]" />
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {results.items.map((item: any) => (
                  <div key={item.id} className="card group">
                    ...
                    <p className="text-xs text-[#E5C158]/60 mb-1">{item.category_name}</p>
                    <h3 className="font-semibold text-white text-sm mb-1">{item.title}</h3>
                    {item.description && <p className="text-xs text-gray-500 line-clamp-2">{item.description}</p>}
                    <div className="flex items-center gap-2 mt-3 pt-3 border-t border-[#E5C158]/10">
                      <Link to="/admin/content" className="btn-ghost text-xs flex items-center gap-1">Editar</Link>
                      {item.video_url && (
                        <Link to={`/player?type=item&id=${item.id}`} className="btn-ghost text-xs text-[#E5C158] flex items-center gap-1 ml-auto">
          ...
          {results.streams.length > 0 && (
            <div>
              <div className="flex items-center gap-2 mb-4">
                <IconPlay className="w-5 h-5 text-[#E5C158]" />
                ...
                {results.streams.map((stream: any) => (
                  <div key={stream.id} className="card group">
                    {stream.thumbnail_url ? (
                      ...
                      <div className="relative -mx-6 -mt-6 mb-4 overflow-hidden h-36 bg-[#111] flex items-center justify-center rounded-t-2xl">
                        <Link to={`/player?type=stream&id=${stream.id}`} className="w-full h-full flex items-center justify-center">
                          <IconPlay className="w-10 h-10 text-[#E5C158]/30 group-hover:text-[#E5C158] transition-colors" />
                        </Link>
                      </div>
                    )}
                    ...
                    <div className="flex items-center gap-2 mt-3 pt-3 border-t border-[#E5C158]/10">
                      ...
                      <Link to={`/player?type=stream&id=${stream.id}`} className="btn-ghost text-xs text-[#E5C158] flex items-center gap-1 ml-auto">
                        <IconPlay className="w-3 h-3" /> Reproducir
                      </Link>
                    </div>
                  </div>
