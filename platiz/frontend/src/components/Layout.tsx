...
import { IconHome, IconMovies, IconCourses, IconBooks, IconApps, IconTelegram, IconAcademy, IconAffiliate, IconChat, IconCog, IconPhoto, IconUsers, IconGrid, IconMenu, IconClose, IconBell, IconSearch, IconChevronDown, IconLogout, IconStar, IconShield } from '../icons/PremiumIcons';
...
const navItems = [
  ...
  { path: '/telegram', label: 'Telegram', icon: IconTelegram, roles: ['client', 'admin'] },
...
export default function Layout() {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0f] overflow-x-hidden">
      <aside className={`fixed top-0 left-0 z-40 h-full w-72 bg-[#050508]/90 lg:bg-[#050508]/90 backdrop-blur-xl border-r border-[#E5C158]/10 transform transition-transform duration-300 lg:translate-x-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`} style={{ WebkitBackdropFilter: 'blur(24px)' }}
        onTouchStart={handleTouchStart}
        onTouchEnd={handleTouchEnd}
      >
        <div className="flex flex-col items-center px-3 py-3 border-b border-[#E5C158]/10">
          <Logo size={28} />
          <span className="text-[10px] text-[#E5C158]/80 uppercase tracking-[3px] mt-1">Transforma el Internet en Dinero</span>
        </div>
        <div className="flex flex-col h-[calc(100vh-4rem)]">
          <nav className="flex-1 p-4 space-y-1 overflow-y-auto">
            {filteredNav.map((item) => {
              ...
              return (
                <Link key={item.path} to={item.path} onClick={() => setSidebarOpen(false)} className={`flex items-center gap-3 px-4 py-3 rounded-xl transition-colors duration-200 group ${isActive ? 'bg-gradient-to-r from-[#E5C158]/15 to-transparent text-[#E5C158] border border-[#E5C158]/15' : 'text-gray-300 hover:text-[#E5C158] hover:bg-[#E5C158]/5 border border-transparent'}`}>
                  <Icon className={`w-5 h-5 ${isActive ? 'text-[#E5C158]' : 'group-hover:text-[#E5C158] transition-colors'}`} />
                  <span className="font-medium">{item.label}</span>
                  {isActive && <div className="ml-auto w-1.5 h-1.5 rounded-full bg-[#E5C158] shadow-[0_0_6px_rgba(255,215,0,0.8)]" />}
                </Link>
              );
            })}
          </nav>
          ...
          {isAdmin && (
            <div className="px-4 pb-4">
              <div className="h-px bg-gradient-to-r from-transparent via-[#E5C158]/15 to-transparent mb-4" />
              <p className="px-4 text-xs font-semibold uppercase tracking-wider text-[#E5C158]/70 mb-2">Administración</p>
              <nav className="space-y-1 max-h-44 overflow-y-auto">
                {adminItems.map((item) => {
                  const isActive = location.pathname === item.path;
                  const Icon = item.icon;
                  return (
                <Link key={item.path} to={item.path} onClick={() => setSidebarOpen(false)} className={`flex items-center gap-3 px-4 py-3 rounded-xl transition-colors duration-200 group ${isActive ? 'bg-[#E5C158]/10 text-[#E5C158] border border-[#E5C158]/15 shadow-[0_2px_8px_rgba(255,215,0,0.06)]' : 'text-gray-300 hover:text-[#E5C158] hover:bg-white/[0.03] border border-transparent'}`}>
                      <Icon className={`w-5 h-5 ${isActive ? 'text-[#E5C158]' : 'group-hover:text-[#E5C158] transition-colors'}`} />
                      <span className="font-medium">{item.label}</span>
                      {isActive && <div className="ml-auto w-1.5 h-1.5 rounded-full bg-[#E5C158] shadow-[0_0_6px_rgba(255,215,0,0.8)]" />}
                    </Link>
      ...
      <div className="lg:pl-72 transition-colors duration-300 min-h-screen overflow-x-hidden">
        <header className="sticky top-0 z-30 bg-[#0a0a0f]/80 backdrop-blur-xl border-b border-[#E5C158]/10">
          <div className="flex items-center justify-between h-11 md:h-16 px-2 md:px-8 gap-0.5">
            <button onClick={() => setSidebarOpen(!sidebarOpen)} className="lg:hidden p-1.5 rounded-lg hover:bg-[#E5C158]/5 text-gray-300 hover:text-[#E5C158] transition-colors flex-shrink-0" aria-label="Menú principal" aria-expanded={sidebarOpen}>
              {sidebarOpen ? <IconClose className="w-5 h-5" /> : <IconMenu className="w-5 h-5" />}
            </button>
            <div className="flex items-center gap-0.5 md:gap-2 px-1.5 md:px-4 py-1 md:py-2 bg-[#0a0a0f]/50 rounded-lg md:rounded-xl border border-[#E5C158]/10 flex-1 max-w-full md:max-w-md">
              <IconSearch className="w-3 h-3 md:w-5 md:h-5 text-gray-300 flex-shrink-0" />
              <input type="text" placeholder="Buscar..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter' && searchQuery.trim()) { navigate(isAdmin ? `/admin/search?q=${encodeURIComponent(searchQuery.trim())}` : `/search?q=${encodeURIComponent(searchQuery.trim())}`); setSearchQuery(''); } }} className="bg-transparent border-none outline-none text-[11px] md:text-sm text-white placeholder-gray-400 w-full min-w-0" />
            </div>
            <div className="flex items-center gap-0.5 md:gap-2 flex-shrink-0">
              <div className="relative">
                <button onClick={() => setUserMenuOpen(!userMenuOpen)} className="flex items-center gap-0.5 md:gap-2 p-1 rounded-lg md:rounded-xl hover:bg-[#E5C158]/5 transition-colors cursor-pointer" aria-haspopup="true" aria-expanded={userMenuOpen}>
                  <div className="w-6 h-6 md:w-8 md:h-8 bg-gradient-to-br from-[#C4A44A] to-[#A6842C] rounded-lg flex items-center justify-center text-black text-[10px] md:text-sm font-bold">{user?.username?.charAt(0).toUpperCase()}</div>
                  <IconChevronDown className="w-3 h-3 md:w-4 md:h-4 text-gray-300 flex-shrink-0" />
                </button>
                {userMenuOpen && (
                  <>
                    <div className="fixed inset-0 z-10" onClick={() => setUserMenuOpen(false)} />
                    <div className="absolute right-0 mt-2 w-56 bg-[#0a0a0f] border border-[#E5C158]/15 rounded-xl shadow-2xl shadow-black/50 z-20 overflow-hidden animate-slide-down">
                      <div className="p-4 border-b border-[#E5C158]/10">
                        <p className="text-sm font-medium text-white">{user?.username}</p>
                      </div>
                      <div className="p-2">
                        <button onClick={() => { logout(); navigate('/login'); setUserMenuOpen(false); }} className="flex items-center gap-3 w-full px-3 py-2.5 rounded-lg text-gray-300 hover:text-[#E5C158] hover:bg-[#E5C158]/5 transition-colors text-sm">
