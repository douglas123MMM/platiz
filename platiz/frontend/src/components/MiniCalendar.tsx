  ...
  return (
    <div className="relative">
      <button type="button" onClick={() => setOpen(!open)}
        className="w-full px-4 py-2.5 bg-[#0a0a0f] border border-white/10 rounded-xl text-white text-sm text-left hover:border-[#E5C158]/30 focus:border-[#E5C158]/40 focus:outline-none transition-colors flex items-center justify-between">
      ...
      {open && (
        <>
          ...
          <div className="absolute z-50 mt-2 w-72 bg-[#0a0a0f] border border-[#E5C158]/15 rounded-2xl shadow-2xl shadow-black/60 p-4 animate-slide-down">
                ...
                return (
                  <button key={day} type="button" disabled={disabled}
                    ...
                    className={`text-center py-2 text-sm rounded-xl transition-all duration-150 font-medium
                      ...
                      ${isSelected(day) ? 'bg-[#E5C158] text-black font-bold shadow-[0_0_12px_rgba(255,215,0,0.3)]' : ''}
                      ${!isSelected(day) && !disabled ? 'text-gray-300 hover:bg-white/[0.06] hover:text-white' : ''}
                      ${isToday(day) && !isSelected(day) ? 'border border-[#E5C158]/30' : ''}`}>
                    {day}
                  </button>
                );
              })}
